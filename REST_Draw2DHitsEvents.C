...
Int_t REST_2DHitsEvent(TString fName, Int_t firstEvent = 1)
{
	...
	TRest2DHitsEvent *evt = new TRest2DHitsEvent();
	...
	TCanvas *c1;
	//evt->PrintEvent();
	for (int i = 0; i < run->GetfEntriesSaved(); i++){
		sprintf(name,"h%d",i);
        sprintf(title,"histo nr:%d",i);
        sprintf(pic_name,"Nevent%d.pdf",i);
			/* code */
		run->GetEntry(i);
		c1 = new TCanvas(name,title,10,10,800,600);
		//c1->Divide(3,3);
